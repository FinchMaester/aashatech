 ...
 <!-- Content Wrapper. Contains page content -->
        ...
        <div class="row mb-2">
          <div class="col-sm-6">
           ...
           <a href="{{ url('admin/message/create') }}"><button class="btn btn-primary btn-sm"><i class="fa fa-plus"></i>Add Message</button></a> 
    ...
    <tbody>
        @foreach ($messages as $message)
            <tr data-widget="expandable-table" aria-expanded="false">
                ...
                <td>{{ $message->name ?? '' }}</td>
                <td>{{ $message->title ?? '' }}</td>
                <td><img src="{{ url('uploads/message/' . $message->image) ?? '' }}" id="preview"
                        style="max-width: 150px; max-height:150px" /></td>
                <td>{{ $message->description ?? '' }}</td>
              
                <td>
                    <a href="/admin/message/edit/{{ $message->id }}">
